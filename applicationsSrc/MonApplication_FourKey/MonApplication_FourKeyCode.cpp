/**
 * MonApplication_FourKeyCode.cpp
 * Implémentation du protocole AKC-PM (Four-Key) + Reconfiguration
...
MonApplication_FourKeyCode::MonApplication_FourKeyCode(Catoms3DBlock *host)
    : Catoms3DBlockCode(host), module(host) {}

MonApplication_FourKeyCode::~MonApplication_FourKeyCode() {}
...
vector<uint8_t> MonApplication_FourKeyCode::H(const vector<uint8_t>& data) {
    return Spongent::Spongent160::hash(data);
}

// H(uint64_t) → 160 bits  (surcharge pratique)
vector<uint8_t> MonApplication_FourKeyCode::H(uint64_t val) {
    vector<uint8_t> v(8);
...
vector<uint8_t> MonApplication_FourKeyCode::L(int ligne) {
...
vector<uint8_t> MonApplication_FourKeyCode::HL(const vector<uint8_t>& n) {
    uint64_t val = 0;
...
vector<uint8_t> MonApplication_FourKeyCode::HL(uint64_t n) {
    return H(L((int)(n % NB_LIGNES_CODE)));
}

// XOR octet par octet (160 bits)
vector<uint8_t> MonApplication_FourKeyCode::xorVec(const vector<uint8_t>& a, const vector<uint8_t>& b) {
...
vector<uint8_t> MonApplication_FourKeyCode::genererNonce160() {
...
vector<uint8_t> MonApplication_FourKeyCode::tsVersVec(uint64_t ts) {
...
void MonApplication_FourKeyCode::afficherStatsGlobal() const {
    ...
    cout << "╠════════════════════════════════════════════════════════════════╣" << endl;
    cout << "║ Modèle de clé        : FOUR_KEY                                ║" << endl;
    cout << "║ Nombre de clés       : 4 (K1, K2, K3, K4)                     ║" << endl;
    cout << "║ Distribution         : Cyclique (mod 4)                        ║" << endl;
    cout << "║ Génération           : K2=H(K1), K3=H(K2), K4=H(K3)           ║" << endl;
    cout << "╠════════════════════════════════════════════════════════════════╣" << endl;
    ...
    cout << "║ Complexité Algo1     : O(m) — " << authenticatedModules.size() 
         << " exécutions                     ║" << endl;
    cout << "║ Overhead par lien    : 6H + 2-4H + 2Tx + 4XOR                  ║" << endl;
    cout << "╚════════════════════════════════════════════════════════════════╝" << endl;
    cout << "\n" << flush;
}
...
void MonApplication_FourKeyCode::startup() {
    ...
    if (module->position == Cell3DPosition(2,2,2)) {
        ...
        console << "    Modele: AKC-PM (FOUR_KEY)\n";
 ...
 */
void MonApplication_FourKeyCode::algorithme1_Initier(P2PNetworkInterface* dest) {
 ...
 *   Génère n2 (FOUR_KEY : iM génère n2, puis le propage aux sM authentifiés)
 *   x1 = n2 XOR n0'
 *   K1 = H(L(n2 mod Nb))
 * Envoie x1 à nM
 */
void MonApplication_FourKeyCode::algorithme1_Verifier(P2PNetworkInterface* src, int liens,
    ...
    // FOUR_KEY: Déterminer le numéro de clé (1-4, cyclique)
    int numCle = (infoSecurite.liensStructure % 4) + 1;
    
    // Générer Ki en appliquant H() numCle-1 fois sur K1
    vector<uint8_t> K1 = HL(infoSecurite.n2);
    vector<uint8_t> Ki = K1;
    
    for (int i = 1; i < numCle; i++) {
        Ki = H(Ki);  // K2=H(K1), K3=H(K2), K4=H(K3)
    }
    
    console << "    FOUR_KEY: Generation de K" << numCle << "\n";
    console << "    K" << numCle << " = ";
    for (int i = 1; i < numCle; i++) console << "H(";
    console << "K1";
    for (int i = 1; i < numCle; i++) console << ")";
    console << "\n";
    console << "    Application de H() " << (numCle - 1) << " fois\n";
    ...
    console << "    Cle K" << numCle << " etablie pour ce lien\n";
    console << "    Message KEY_CHALLENGE envoye (x1 = n2 XOR n0)\n";
    
    // FOUR_KEY: Définir la couleur selon le numéro de clé
    // K1=CYAN, K2=YELLOW, K3=MAGENTA, K4=ORANGE
    Color couleurCle;
    switch(numCle) {
        case 1: couleurCle = CYAN; break;
        case 2: couleurCle = YELLOW; break;
        case 3: couleurCle = MAGENTA; break;
        case 4: couleurCle = ORANGE; break;
        default: couleurCle = GREEN; break;
    }
    // Note: La couleur sera appliquée par le module qui reçoit la clé
 ...
 *   n2 = x1 XOR n0
 *   K1 = H(L(n2 mod Nb))
 * → nM et iM partagent K1 (FOUR_KEY)
 * IMPORTANT: nM stocke n2 pour pouvoir authentifier de futurs voisins (modèle FOUR_KEY)
 */
void MonApplication_FourKeyCode::algorithme1_Completer(P2PNetworkInterface* src,
    ...
    // FOUR_KEY: Déterminer notre numéro de clé
    int numCle = (infoSecurite.liensStructure % 4) + 1;
    ...
    console << "    FOUR_KEY: K" << numCle << " = ";
    for (int i = 1; i < numCle; i++) console << "H(";
    console << "K1";
    for (int i = 1; i < numCle; i++) console << ")";
    console << " calculee\n";

    // FOUR_KEY: Stocker n2 pour pouvoir authentifier de futurs modules
    // Tous les modules de la structure partagent le même n2
    infoSecurite.n2           = n2;
    infoSecurite.K1           = Ki;  // Stocker Ki (pas K1)
    infoSecurite.numeroClé    = numCle;
    clesVoisins[src]          = Ki;
    voisinsAuthentifies[src]  = true;
    voisinsDansStructure[src] = true;
    infoSecurite.etat         = ETAT_CLE_ETABLIE;
    ...
    console << "    Cle K" << numCle << " etablie (FOUR_KEY)\n";
    console << "    4 cles distinctes: K1, K2=H(K1), K3=H(K2), K4=H(K3)\n";
    ...
    console << "    Couleur du module: K" << numCle << " (";
    switch(numCle) {
        case 1: console << "CYAN"; break;
        case 2: console << "YELLOW"; break;
        case 3: console << "MAGENTA"; break;
        case 4: console << "ORANGE"; break;
    }
    console << ")\n";
 ...
 */
void MonApplication_FourKeyCode::algorithme2_Initier(P2PNetworkInterface* dest) {
 ...
 */
void MonApplication_FourKeyCode::algorithme2_Relayer(P2PNetworkInterface* src, int liens,
    ...
    if (liens > 0) {
        for (int i = 0; i < 12; i++) {
            ...
            // Utiliser la clé partagée si elle existe, sinon K1 (FOUR_KEY)
            vector<uint8_t> Kv;
            if (clesVoisins.count(iface)) {
                Kv = clesVoisins[iface];
            } else if (!infoSecurite.K1.empty()) {
                // FOUR_KEY: utiliser K1 pour les voisins non encore authentifiés
                Kv = infoSecurite.K1;
            } else {
 ...
 */
void MonApplication_FourKeyCode::algorithme2_Verifier(P2PNetworkInterface* src, int /*liens*/,
                                               const vector<uint8_t>& n1) {
 ...
 *   Envoie x1 à 1M
 */
void MonApplication_FourKeyCode::algorithme2_Completer(P2PNetworkInterface* src,
...
void MonApplication_FourKeyCode::notifierVoisinsStructurePrete() {
    uint64_t t = getScheduler()->now();
...
void MonApplication_FourKeyCode::verifierNouveauxVoisins() {
    if (!estDansStructure()) return;
    for (int i = 0; i < 12; i++) {
        P2PNetworkInterface* iface = module->getInterface(i);
        if (!ifaceValide(iface, module)) continue;
...
void MonApplication_FourKeyCode::lancerReconfiguration() {
    ...
    MonApplication_FourKeyCode* nc = (MonApplication_FourKeyCode*)nextBlock->blockCode;
    cout << "  Module " << nextId << " : ("
...
void MonApplication_FourKeyCode::surReceptionDemandeAuth(shared_ptr<Message> msg,
                                                  P2PNetworkInterface* src) {
    auto* m = static_cast<MessageDemandeAuth*>(msg.get());
    algorithme1_Verifier(src, m->liens, m->n1, m->x, m->K0);
}

void MonApplication_FourKeyCode::surReceptionDefiCle(shared_ptr<Message> msg,
                                              P2PNetworkInterface* src) {
...
void MonApplication_FourKeyCode::surReceptionStructurePrete(shared_ptr<Message> /*msg*/,
                                                     P2PNetworkInterface* src) {
    voisinsDansStructure[src] = true;
...
void MonApplication_FourKeyCode::surReceptionPreuveMembership(shared_ptr<Message> msg,
                                                       P2PNetworkInterface* src) {
    auto* m = static_cast<MessagePreuveMembership*>(msg.get());
    algorithme2_Verifier(src, m->liens, m->n1);
}

void MonApplication_FourKeyCode::surReceptionRelaiPreuve(shared_ptr<Message> msg,
                                                  P2PNetworkInterface* src) {
...
void MonApplication_FourKeyCode::processLocalEvent(EventPtr pev) {
...
bool MonApplication_FourKeyCode::tryMoveToward(const Cell3DPosition& goal) {
...
void MonApplication_FourKeyCode::onMotionEnd() {
...
void MonApplication_FourKeyCode::parseUserBlockElements(TiXmlElement* /*config*/) {}
