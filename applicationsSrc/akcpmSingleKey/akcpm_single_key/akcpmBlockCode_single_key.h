...
/**
 * Données de mouvement pour un module
 * Définit un mouvement de posFrom vers posTo à une étape (stage) donnée
 */
struct ModuleMovement {
    Cell3DPosition posFrom;   // Position de départ
    Cell3DPosition posTo;     // Position d'arrivée
    uint16_t stage;           // Étape du mouvement (0, 1, 2, ...)
...
/**
 * MSG_MOTION_SYNC: Synchronisation des mouvements entre modules
 * Contenu: (stage, allAuthenticated)
 */
static const int MSG_MOTION_SYNC = 2004;
...
private:
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // ATTRIBUTS POUR LES MOUVEMENTS
    // ═══════════════════════════════════════════════════════════════════════
    
    FCCLattice* lattice;                      // Référence au réseau FCC
    bool isMobile = false;                    // Module mobile ?
    int myStage = 0;                          // Étape actuelle du mouvement
    int myMaxStage = 0;                       // Nombre total d'étapes
    bool reconfigurationActive = false;       // Phase de reconfiguration active ?
    
    // Configuration initiale pour reset
    static std::map<bID, std::pair<Cell3DPosition, uint8_t>> configurationInitiale;

public:
    // Plan de mouvements (public pour accès depuis main)
...
private:
    ...
    /**
     * Sauvegarde la configuration initiale pour pouvoir la restaurer
     */
     ...
     * @return true si le mouvement a été initié
     */
    bool tryToMove(const Cell3DPosition& targetPos);
    
    /**
     * Vérifie si tous les modules sont authentifiés et lance la reconfiguration
     */
    void verifierEtLancerReconfiguration();
    
    /**
     * Cherche et exécute le mouvement pour l'étape courante
     */
    void executerMouvementEtape();
    
    /**
     * Gestionnaire de message de synchronisation des mouvements
     */
    void surReceptionMotionSync(std::shared_ptr<Message> msg, P2PNetworkInterface* src);
...
public:
    ...
    /**
     * Parse les attributs personnalisés des blocs dans le fichier XML
     * Notamment l'attribut "mobile"
     */
...
/**
 * Message de synchronisation des mouvements
 * Permet de synchroniser les étapes de mouvement entre modules
 */
class MessageMotionSync : public Message {
public:
    int stage;                  // Étape actuelle
    bool allAuthenticated;      // Tous les modules sont authentifiés ?
    
    MessageMotionSync(int _stage, bool _allAuth);
    ~MessageMotionSync() override;
};
