#ifndef MonApplication_MultiKeyCode_H_
#define MonApplication_MultiKeyCode_H_
...
struct InfoSecurite {
    ...
    std::vector<uint8_t> n2;            // Nonce de génération de clé (partagé en MULTI_KEY)
    ...
    std::vector<uint8_t> K1;            // H(L(n2 mod Nb)) — clé partagée MULTI_KEY
                                        // Tous les modules authentifiés partagent la même K1
    
    // MULTI_KEY: Gestion des clés multiples
    std::vector<uint8_t> derniereClé;   // Dernière clé générée (pour Ki+1 = H(L(Ki mod Nb)))
    int compteurClés = 0;                // Nombre de clés générées par ce module
};
...
class MonApplication_MultiKeyCode : public Catoms3DBlockCode {
private:
...
public:
    MonApplication_MultiKeyCode(Catoms3DBlock *host);
    ~MonApplication_MultiKeyCode();
    ...
    static BlockCode *buildNewBlockCode(BuildingBlock *host) {
        return new MonApplication_MultiKeyCode((Catoms3DBlock*)host);
    }
};
