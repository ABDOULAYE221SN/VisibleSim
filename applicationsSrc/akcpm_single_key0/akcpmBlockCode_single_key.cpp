...
void AkcpmBlockCode::definirCommeModuleInitial() {
    ...
    console << "  Modèle: SINGLE_KEY (cle unique pour tout le reseau)\n";
}
...
void AkcpmBlockCode::startup() {
    ...
    console << "Module " << hostBlock->blockId << " - Protocole AKC-PM (SINGLE KEY)\n";
    
    Catoms3DBlock* catom = (Catoms3DBlock*)hostBlock;
    Cell3DPosition pos = catom->position;
    
    console << "  Position: (" << pos[0] << "," << pos[1] << "," << pos[2] << ")\n";
    console << "  Voisins connectes: " << compterVoisinsConnectes() << "\n";
...
void AkcpmBlockCode::algorithme1_Verifier(P2PNetworkInterface* src, int liens, 
    ...
    console << "Module " << hostBlock->blockId << ": ALGORITHME 1 - VERIFICATION (SF-3, SF-4)\n";
    ...
    if (!syncOK) {
        ...
        console << "    Plage testee: Ts de " << (Ts_base - TOLERANCE_SYNC) 
                << " a " << (Ts_base + TOLERANCE_SYNC) << "\n";
        return;
    }
    ...
    if (K0_calcule != K0) {
        console << "   ECHEC AUTH: K0 ne correspond pas (empreinte invalide)\n";
        return;
    }
    ...
    if (infoSecurite.estModuleInitial && !n2GlobalDefini) {
        ...
        n2GlobalDefini = true;
        console << "  [SINGLE KEY] n2 global genere par le module initial\n";
    }
    n2 = n2Global;
    ...
    console << "  K1 = H(L(n2 mod Nb)) - cle unique pour tout le reseau\n";
    ...
    console << "  → Message KEY_CHALLENGE envoye (x1)\n";
...
void AkcpmBlockCode::algorithme1_Completer(P2PNetworkInterface* src, uint64_t x1) {
    ...
    console << "Module " << hostBlock->blockId << ": ALGORITHME 1 - Complete l'authentification (SF-5)\n";
    ...
    console << "  Module integre a la structure! \n";
    console << "  Cle K1 partagee etablie (SINGLE_KEY) \n";
    console << "  Cette cle K1 est identique pour tout le reseau \n";
    ...
    for (int i = 0; i < 12; i++) {
        ...
        if (iface && iface->connectedInterface) {
            ...
            if (voisinsDansStructure.find(iface) == voisinsDansStructure.end() &&
                ...
                if (estDansStructure()) {
                    console << "Module " << hostBlock->blockId << ": Nouveau voisin detecte\n";
                    MessageStructurePrete* msg = new MessageStructurePrete();
                    sendMessage(msg, iface, DELAI_TRANSMISSION, 0);
                }
            }
...
void AkcpmBlockCode::surReceptionDemandeAuth(std::shared_ptr<Message> msg, 
    ...
    console << "  n1 = " << authMsg->n1 << "\n";
    // Affichage hexadécimal de x||K0 concaténés
    console << "  x||K0 = 0x";
    for (int i = 7; i >= 0; i--) {
        int byte = (authMsg->x >> (i * 8)) & 0xFF;
        if (byte < 16) console << "0";
        console << std::hex << byte;
    }
    for (auto byte : authMsg->K0) {
        if ((int)byte < 16) console << "0";
        console << std::hex << (int)byte;
    }
    console << std::dec << "\n";
    ...
    switch (pev->eventType) {
        ...
        case EVENT_INTERRUPTION: {
            ...
            if (infoSecurite.estModuleInitial) {
                console << "Module " << hostBlock->blockId << ": Demarrage du protocole\n";
                notifierVoisinsStructurePrete();
            }
            break;
        }
...
void AkcpmBlockCode::onTap(int face) {
    ...
    console << " Modele de cle: SINGLE_KEY (cle unique K1)\n";
}
